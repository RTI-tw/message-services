...
# export PUBSUB_TOPIC_BOOKMARK="dev-forum-bookmark-events"
...
gcloud pubsub topics create "${PUBSUB_TOPIC_BOOKMARK}"
```
...
- 建立上述四個 topics（若已存在會略過）
- 以及對應的四個 subscriptions（預設名稱為 `<topic>-sub`，可用環境變數 `PUBSUB_SUB_*` 覆寫）
...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --set-env-vars "GCP_PROJECT_ID=${PROJECT_ID},PUBSUB_TOPIC_POST=${PUBSUB_TOPIC_POST},PUBSUB_TOPIC_COMMENT=${PUBSUB_TOPIC_COMMENT},PUBSUB_TOPIC_REACTION=${PUBSUB_TOPIC_REACTION},PUBSUB_TOPIC_BOOKMARK=${PUBSUB_TOPIC_BOOKMARK},KEYSTONE_GQL_ENDPOINT=${KEYSTONE_GQL_ENDPOINT},KEYSTONE_AUTH_TOKEN=${KEYSTONE_AUTH_TOKEN},GEMINI_API_KEY=${GEMINI_API_KEY},GEMINI_MODEL=${GEMINI_MODEL}"
```
...
- 發佈事件：`POST /post/create`、`/post/update`、`/comment/create`、`/comment/update`、`/reaction/create`、`/reaction/update`、`/bookmark/create`、`/bookmark/update`
...
- 若 **尚未建立** 過 subscription：腳本會建立四個 **Push** subscription，並指向 `PUBSUB_PUSH_ENDPOINT`。
- 若 **已存在** subscription：
